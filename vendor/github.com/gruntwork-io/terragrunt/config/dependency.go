...
func decodeAndRetrieveOutputs(
	...
	if trackInclude != nil {
		...
		if err != nil {
			fmt.Println("^^^^ handleIncludeForDependency")
			return nil, err
		}
	...
	if err := checkForDependencyBlockCycles(filename, decodedDependency, terragruntOptions); err != nil {
		fmt.Println("^^^^ checkForDependencyBlockCycles")
		return nil, err
	}
...
func dependencyBlocksToCtyValue(dependencyConfigs []Dependency, terragruntOptions *options.TerragruntOptions) (*cty.Value, error) {
	...
	for _, dependencyConfig := range dependencyConfigs {
		...
		dependencyErrGroup.Go(func() error {
			...
			if err := dependencyConfig.setRenderedOutputs(terragruntOptions); err != nil {
				fmt.Printf("dependencyConfig.setRenderedOutputs %+v, %+v\n", terragruntOptions, err)
				return err
			}
			...
			if err != nil {
				...
				fmt.Println("gocty.ToCtyValue", paths, err)
				return err
			}
	...
	if err := dependencyErrGroup.Wait(); err != nil {
		fmt.Println("error group waittttttt", err)
		return nil, err
	}
...
func getTerragruntOutputIfAppliedElseConfiguredDefault(dependencyConfig Dependency, terragruntOptions *options.TerragruntOptions) (*cty.Value, error) {
	if dependencyConfig.shouldGetOutputs() {
		...
		if err != nil {
			fmt.Println("getTerragruntOutputIfAppliedElseConfiguredDefault getTerragruntOutput: ", err)
			return nil, err
		}
	...
	fmt.Println("getTerragruntOutputIfAppliedElseConfiguredDefault skip output: ", err)
	return nil, err
}
...
func getTerragruntOutput(dependencyConfig Dependency, terragruntOptions *options.TerragruntOptions) (*cty.Value, bool, error) {
	...
	if !util.FileExists(targetConfig) {
		fmt.Println("getTerragruntOutput: !FileExists")
	...
	if err != nil {
		fmt.Println("getTerragruntOutput: getOutputJsonWithCaching")
		return nil, true, err
	}
	...
	if err != nil {
		fmt.Println("getTerragruntOutput: terraformOutputJsonToCtyValueMap")
	...
	fmt.Println("getTerragruntOutput: TerragruntOutputEncodingError")
...
func getTerragruntOutputJson(terragruntOptions *options.TerragruntOptions, targetConfig string) ([]byte, error) {
	...
	if err != nil {
		fmt.Println("cloneTerragruntOptionsForDependencyOutput error:", err)
		return nil, err
	}
	...
	if err != nil || !canGetRemoteState(remoteStateTGConfig.RemoteState) {
		...
		fmt.Println("PartialParseConfigFile error:", err)
	...
	if err != nil {
		fmt.Println("terragruntAlreadyInit error:", err)
		return nil, err
	}
	if isInit {
		fmt.Println("isInit")
...
func getTerragruntOutputJsonFromInitFolder(terragruntOptions *options.TerragruntOptions, terraformWorkingDir string, iamRoleOpts options.IAMRoleOptions) ([]byte, error) {
	...
	if err != nil {
		fmt.Println("setupTerragruntOptionsForBareTerraform", err)
		return nil, err
	}
	out, err := shell.RunTerraformCommandWithOutput(targetTGOptions, "output", "-json")
	fmt.Printf("\n\ninit folder: %+v\n\n", targetTGOptions)
	fmt.Printf("\n\n%+v\n\n%+v\n\n", targetTGOptions.OriginalIAMRoleOptions, targetTGOptions.IAMRoleOptions)
	if err != nil {
		fmt.Printf("\n\n%+v\n\n", targetTGOptions)
		fmt.Println("RunTerraformCommandWithOutput", out, err)
		return nil, err
	}
