...
func CreateTerragruntEvalContext(
	...
	if extensions.TrackInclude != nil && len(extensions.TrackInclude.CurrentList) > 0 {
		...
		if err != nil {
			fmt.Println("==== includeMapAsCtyVal arguments:", extensions.TrackInclude.CurrentMap, terragruntOptions, extensions.DecodedDependencies, extensions.PartialParseDecodeList)
			fmt.Println("#### includeMapAsCtyVal", extensions.DecodedDependencies == nil)
			fmt.Println(err)
			return ctx, err
		}
