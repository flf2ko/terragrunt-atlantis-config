...
func includeMapAsCtyVal(
	...
	if len(includeMap) == 1 && hasBareInclude {
		...
		// fmt.Println("Detected single bare include block - exposing as top level")
	...
	for key, included := range includeMap {
		...
		if err != nil {
			...
			fmt.Println(decodedDependencies == nil)
			// fmt.Println(decodeList)
			fmt.Println(key)
		...
		if parsedIncludedCty != cty.NilVal {
			...
			// fmt.Printf("Exposing include block '%s'", key)
...
func includeConfigAsCtyVal(
	...
	if includeConfig.GetExpose() {
		...
		if err != nil {
			fmt.Println("parseIncludedConfig", decodedDependencies == nil)
			return cty.NilVal, err
		}
		parsedIncludedCty, err := TerragruntConfigAsCty(parsedIncluded)
		if err != nil {
			fmt.Println("TerragruntConfigAsCty")
			return cty.NilVal, err
		}
